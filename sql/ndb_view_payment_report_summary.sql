...
DROP VIEW IF EXISTS `ndb_view_payment_report_summary`$$

CREATE ALGORITHM=UNDEFINED DEFINER=`ndb`@`%` SQL SECURITY DEFINER VIEW `ndb_view_payment_report_summary` AS 
...
(CASE WHEN (`pr`.`taskid` IS NOT NULL) THEN `pr`.`amount` ELSE `ptp`.`amount` END) AS `amount`,`t`.`adj_type` AS `adj_type`,
(CASE WHEN (`pr`.`taskid` IS NOT NULL) THEN `t`.`process_type` ELSE 0 END) AS `paid`,
...
(CASE WHEN (`pr`.`taskid` IS NOT NULL) THEN `pr`.`payment_type` ELSE `ptp`.`type` END) AS `payment_type`,
...
FROM ((((((((((((((`ndb_project_task` `pt` 
	...
	LEFT JOIN `ndb_employees` `e` ON((`e`.`id` = `pte`.`uid`))) LEFT JOIN `ndb_employees` `e2` ON((`e2`.`id` = IFNULL(`pct`.`taskmanagerid`,`ptc`.`manageuid`)))) 
	LEFT JOIN `ndb_location` `nl` ON((`nl`.`id` = `ba`.`bank_location`))) WHERE ((`p`.`category` <> 5) AND (`pt`.`status` = 8)
)$$
